...
public class GenerateFence : MonoBehaviour
{
    ...
    void Generate()
    {
        ...
        for (var i = 0; i < amount; i++)
        {
            Instantiate(stem, new Vector3(transform.localPosition.x + 999, transform.localPosition.y + offset.y, i * -4.75f + -offset.z), Quaternion.Euler(0, 90, 0), gameObject.transform);

        }
        for (var i = 0; i < amount; i++)
        {
            Instantiate(stem, new Vector3(transform.localPosition.x + 1f, transform.localPosition.y + offset.y, i * -4.75f + -offset.z), Quaternion.Euler(0, 90, 0), gameObject.transform);
